...
if TPTMP then if TPTMP.version <= 7 then TPTMP.disableMultiplayer() else error("newer version already running") end end local get_name = tpt.get_name -- if script already running, replace it
TPTMP = {["version"] = 7, ["versionStr"] = versionstring} -- script version sent on connect to ensure server protocol is the same
...
local function conSend(cmd,msg,str8)
	if not con.connected then return false,"Not connected" end
	msg = msg or ""
	if str8 then msg = string.char(math.min(#msg, 255))..msg:sub(1, 255) end
...
local function connectToServer(ip,port,nick)
	...
	local connUid = uid or ""
	local connAuthToken = (authToken or ""):sub(1, 255)
	sock:send(string.char(tpt.version.major)..string.char(tpt.version.minor)..string.char(TPTMP.version).."\0"..string.char(#nick)..nick..string.char(#connUid)..connUid..string.char(#connAuthToken)..connAuthToken)
	local c,r,zs
...
new=function(x,y,w,h)
	...
	kick = function(self, msg, args)
		...
		local arg = args[1]:sub(1, 255)
		local concat = table.concat(args, " ", 2):sub(1, 255)
		conSend(21, string.char(#arg)..arg..string.char(#concat)..concat)
	end,
...
local dataCmds = {
	...
	[16] = function()
		...
		for i=1,amount do
			local id = cByte()
			con.members[id]={name=conGetStr8(),mousex=0,mousey=0,brushx=4,brushy=4,brush=0,selectedl=1,selectedr=0,selecteda=296,replacemode=0,dcolour={0,0,0,0},lbtn=false,abtn=false,rbtn=false,ctrl=false,shift=false,alt=false}
	...
	[19] = function()
		chatwindow:addline(con.members[cByte()].name .. ": " .. conGetStr8())
	end,
	[20] = function()
		chatwindow:addline("* "..con.members[cByte()].name .. " " .. conGetStr8())
	end,
	[22] = function()
		chatwindow:addline("[SERVER] "..conGetStr8(), cByte(), cByte(), cByte())
	end,
	--Mouse Position
	[32] = function()
		local id = cByte()
	...
	[128] = function()
		local id = cByte()
		conSend(6,string.char(id,49,tpt.set_pause()))
		...
		local d = #s
		conSend(6,string.char(id,66,0,0,0,math.floor(d/65536),math.floor(d/256)%256,d%256)..s)
		conSend(6,string.char(id,53,tpt.ambient_heat()))
		conSend(6,string.char(id,54,tpt.newtonian_gravity()))
		conSend(6,string.char(id,56,tpt.heat()))
		conSend(6,string.char(id,57,sim.waterEqualisation()))
		conSend(6,string.char(id,58,sim.gravityMode()))
		conSend(6,string.char(id,59,sim.airMode()))
		conSend(6,string.char(id,68,sim.edgeMode()))
...
local function connectThink()
	...
	--ping every minute
	if os.time()>con.pingTime then conSend(3) con.pingTime=os.time()+60 end
end
